...
#################################################################################
header <- dashboardHeaderPlus(title = NULL)
  ...
  tabItems(
    ...
    tabItem(
      ...
      fluidRow(
        box(
          ...
          HTML(paste('<h5><b>','Select columns to split dataset','</b><h6>',
          ...
          tags$hr(),  # Horizontal line ----
          HTML(paste('<h5><b>','Two Options','</b><h6>',
      ...
      fluidRow(
        box(
          ...
          solidHeader = TRUE,
          collapsible = FALSE,
          DT::dataTableOutput('mtblfirst',height = 375)
        ), #box closure
        box(
          width = 6,
          height = 450,
          ...
          solidHeader = TRUE,
          collapsible = FALSE,
          DT::dataTableOutput('mtblsecond',height = 375)
        ) #box closure
      ) #fluidrow closure
    ),
    tabItem(
    ...
    tabItem(
      ...
      box(
        ...
        solidHeader = FALSE,
        collapsible = FALSE,
        plotOutput('mcorrelationplotCircle',height = 450)
      ) #box closure
    ),
    tabItem(
      tabName = "tabpairpanelplot",
      ...
      HTML(paste('<h4><b>',"Correlation Pair-Panel Plot",'</b><h6>',
                 'ALL Columns of Original dataset or only with selected columns','<h5>')),
      plotOutput('mcorrelationpairpanel',height = '450px')
    ),
    tabItem(
      tabName = "tabdata",
      align = "center",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$file,{
    ...
    #updateSelectInput(session,inputId = 'mfirstdfcolumn',choices = colnames(select_if(vmy$mydata, is.numeric)))
    fncreatedftype()
  })
  ...
  observeEvent(input$ok, {
    ...
    if ((ncol(dplyr::select_if(vmy$mydata,is.numeric))==2)==TRUE) {
      removeModal()
  ...
  observeEvent(input$mselectALLbtn,{
    ...
    updateSelectInput(session,inputId = 'mfirstdfcolumn',choices = colnames(vmy$mydata),selected = colnames(vmy$mydata))
  })
  ...
  observeEvent(input$mselectOneByOnebtn,{
    ...
    updateSelectInput(session,inputId = 'mfirstdfcolumn',choices = colnames(vmy$mydata),selected = NULL)
  })
  

  observeEvent(input$mfirstdfcolumn,{
    if (length(input$mfirstdfcolumn)== (ncol(vmy$mydata)-1)){
      shinyalert("Oops!", "you need to remove two columns to make the second dataset as formal dataframe", type = "error")
      
      return()
    }
    
    if (length(input$mfirstdfcolumn)== (ncol(vmy$mydata)-2)){
  ...
  vmy$mfirstdf <-reactive({
    tryCatch({ 
      ...
      mfirstdf <- mfirstdf[ , names(mfirstdf) %in% moptionsTXT]
      mfirstdf
  ...
  vmy$mseconddf <-reactive({
    ...
    tryCatch({ 
      if (isTRUE(dplyr::all_equal(vmy$mfirstdf(),vmy$mydata))){
        ...
        mseconddf <- vmy$mydata
        moptionsTXT <-  input$mfirstdfcolumn 
        mseconddf <- mseconddf[ , names(mseconddf) %!in% moptionsTXT]
      }
    }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
    return(mseconddf)
  })
  ...
  fnCorrelationProcessing <- function(){
    ...
    yyCorreSign <- yyCorreSign[,c(input$mfirstdfcolumn)]
  ...
  fnPvalueProcessing <- function(){
    library(xtable)
    yyCorreSign <-fnpvaluestars(select_if(vmy$mydata, is.numeric))
  ...
  output$mcorrelationplotCircle <- renderPlot({
    ...
    M <-cor(select_if(vmy$mfirstdf(), is.numeric) )
